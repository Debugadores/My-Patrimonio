        ...
        if ($stmt_empresa = mysqli_prepare($conection_db, $sql_empresa)) {
            ...
            if (mysqli_stmt_execute($stmt_empresa)) {
                ...
                if (mysqli_stmt_num_rows($stmt_empresa) == 1) {
                    ...
                    $sql_user = "SELECT id, usuario, password, codigofuncionario, nome FROM users WHERE usuario = ?";
                    if ($stmt_user = mysqli_prepare($conection_db, $sql_user)) {
                        ...
                        if (mysqli_stmt_execute($stmt_user)) {
                            ...
                            if (mysqli_stmt_num_rows($stmt_user) == 1) {
                                // Verificar senha
                                mysqli_stmt_bind_result($stmt_user, $id, $db_usuario, $hashed_password, $codigofuncionario, $nome);
                                if (mysqli_stmt_fetch($stmt_user)) {
                                    if (password_verify($password, $hashed_password)) {
                                        ...
                                        $_SESSION["codigofuncionario"] = $codigofuncionario;
                                        $_SESSION["nome"] = $nome;
...
<script>
    // Remova todas as tags <script> anteriores relacionadas ao jQuery e coloque este código em um arquivo login.js
$(document).ready(function() {
    ...
    $('#empresaCodigo').on('blur', function() {
        ...
        if(codigo) {
            $.ajax({
                ...
                success: function(response) {
                    if(response.success) {
                        ...
                        $('#razaoSocial').removeClass('is-invalid').addClass('is-valid');
                    } else {
                        $('#razaoSocial').val('');
                        $('#empresaCodigo').removeClass('is-valid').addClass('is-invalid');
                        $('#razaoSocial').removeClass('is-valid').addClass('is-invalid');
            ...
            $('#razaoSocial').val('');
            $('#empresaCodigo, #razaoSocial').removeClass('is-valid is-invalid');
        }
    });
    ...
    $('#usuario').on('blur', function() {
        ...
        if(usuario) {
            $.ajax({
                ...
                success: function(response) {
                    if(response.success) {
                        ...
                        $('#funcionario').removeClass('is-invalid').addClass('is-valid');
                    } else {
                        $('#funcionario').val('');
                        $('#usuario').removeClass('is-valid').addClass('is-invalid');
                        $('#funcionario').removeClass('is-valid').addClass('is-invalid');
            ...
            $('#funcionario').val('');
            $('#usuario, #funcionario').removeClass('is-valid is-invalid');
        }
    });
